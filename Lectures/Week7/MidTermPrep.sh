...
head sherlock.txt -n 1000 | grep -w "Holmes" > extract.txt
...
grep -oE "[[:alpha:]]+['’][[:alpha:]]+" sherlock.txt | sort | uniq -c
#3d. Append it to extract.txt
grep -oE "[[:alpha:]]+['’][[:alpha:]]+" sherlock.txt | sort | uniq -c >> extract.txt
